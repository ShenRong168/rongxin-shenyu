...
guard arguments.count == 3 else {
    fputs("Usage: render_waiting_for_free_time_quote_card <input.png> <output.png>\n", stderr)
    exit(2)
}
...
let quoteAttributes: [NSAttributedString.Key: Any] = [
    .font: NSFont(name: "Songti TC", size: 72) ?? NSFont(name: "PingFangTC-Semibold", size: 72) ?? NSFont.systemFont(ofSize: 72, weight: .semibold),
    .foregroundColor: ink,
    .paragraphStyle: quoteParagraph,
    .kern: 1.5
]
(quote as NSString).draw(with: NSRect(x: 155, y: 610, width: 770, height: 230), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: quoteAttributes)
